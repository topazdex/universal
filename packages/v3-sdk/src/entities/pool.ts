...
/**
 * A Topaz CL (Slipstream) pool.
 *
 * Slipstream keys pools by tick spacing, not by fee: one pool exists per
 * `(token0, token1, tickSpacing)` and its swap fee is a mutable property of the pool,
 * driven by the factory's fee module. `fee` here is in pips (1e-6).
 */
export class Pool {
   ...
   * @param fee The pool's swap fee in pips (1e-6)
   * @param tickSpacing The tick spacing the pool is keyed by
  ...
  public constructor(
    ...
    invariant(Number.isInteger(tickSpacing) && tickSpacing > 0, 'TICK_SPACING')
  ...
  /** The default fee the factory assigns to this tick spacing, if it is a standard one */
  public get defaultFee(): number | undefined {
    return DEFAULT_FEE_BY_TICK_SPACING[this.tickSpacing as TickSpacing]
  }
}
