...
import {TopazV2Library} from './TopazV2Library.sol';
...
/// @title Router for Topaz v2 (Solidly volatile & stable) Trades
abstract contract V2SwapRouter is RouterImmutables, Permit2Payments {
    ...
    function _v2Swap(Route[] memory routes, address recipient, address pair) private {
        unchecked {
            ...
            (address token0,) = TopazV2Library.sortTokens(routes[0].from, routes[0].to);
            ...
            for (uint256 i; i < length; i++) {
                ...
                uint256 amountOutput = TopazV2Library.getAmountOut(
                    TOPAZ_V2_FACTORY, pair, amountInput, reserveInput, reserveOutput, routes[i]
                );
                (uint256 amount0Out, uint256 amount1Out) =
                    input == token0 ? (uint256(0), amountOutput) : (amountOutput, uint256(0));
                address nextPair;
                (nextPair, token0) = i < finalPairIndex
                    ? TopazV2Library.pairAndToken0For(
                        TOPAZ_V2_FACTORY,
                        TOPAZ_V2_IMPLEMENTATION,
                        routes[i + 1].from,
                        routes[i + 1].to,
                        routes[i + 1].stable
                    )
    ...
    /// @notice Performs a Topaz v2 exact input swap
    ...
    function v2SwapExactInput(
        ...
        address firstPair = TopazV2Library.pairFor(
            TOPAZ_V2_FACTORY, TOPAZ_V2_IMPLEMENTATION, routes[0].from, routes[0].to, routes[0].stable
        );
        if (
    ...
    /// @notice Performs a Topaz v2 exact output swap
    /// @param recipient The recipient of the output tokens
