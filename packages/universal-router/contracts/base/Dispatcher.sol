...
import {V2SwapRouter} from '../modules/topaz/v2/V2SwapRouter.sol';
import {V3SwapRouter} from '../modules/topaz/v3/V3SwapRouter.sol';
import {BytesLib} from '../modules/topaz/v3/BytesLib.sol';
        ...
        if (command < Commands.SECOND_IF_BOUNDARY) {
            ...
            if (command < Commands.FIRST_IF_BOUNDARY) {
                if (command == Commands.V3_SWAP_EXACT_IN) {
                    ...
                    if (value == Constants.CONTRACT_BALANCE) value = ERC20(token).balanceOf(lockedBy);
                    payOrPermit2Transfer({token: token, payer: lockedBy, recipient: map(recipient), amount: value});
                }
                // 0x08 <= command < 0x10
            } else {
            ...
            if (command == Commands.EXECUTE_SUB_PLAN) {
                ...
                // placeholder area for commands 0x10-0x20 and 0x22-0x3f
                revert InvalidCommandType(command);
            }
        }
    }
