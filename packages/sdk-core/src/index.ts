...
import { getChainConfig, wrappedNativeOnChain } from './chains'

/** BNB Chain mainnet */
export const TOPAZ_CHAIN_ID = 56

export const WBNB = new Token(TOPAZ_CHAIN_ID, '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c', 18, 'WBNB', 'Wrapped BNB')
...
export class BNB extends NativeCurrency {
  ...
  public static onChain(chainId: number): BNB {
    invariant(chainId === TOPAZ_CHAIN_ID, 'BNB_CHAIN')
...
class ChainNativeCurrency extends NativeCurrency {
  private readonly wrappedToken: Token
  public constructor(chainId: number) {
    const config = getChainConfig(chainId)
    super(chainId, config.nativeCurrency.decimals, config.nativeCurrency.symbol, config.nativeCurrency.name)
    this.wrappedToken = wrappedNativeOnChain(chainId)
  }
  public get wrapped(): Token {
...
export function nativeOnChain(chainId: number = TOPAZ_CHAIN_ID): NativeCurrency {
  return chainId === TOPAZ_CHAIN_ID ? BNB.onChain(chainId) : new ChainNativeCurrency(chainId)
}
